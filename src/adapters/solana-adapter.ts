...
/** Native system-account data size for rent-exempt minimum lookups. */
const SOL_SYSTEM_ACCOUNT_SPACE = 0n;
...
export class SolanaAdapter implements ChainAdapter {
  ...
  async buildUnsignedTransfer(params: ChainAdapterTransferParams): Promise<UnsignedArtifact> {
    ...
    if (params.asset === native.symbol) {
      ...
      if (!recipientExists) {
        const rentExemptMin = await this.provider.getMinimumBalanceForRentExemption(
          SOL_SYSTEM_ACCOUNT_SPACE,
        );
        if (params.rawAmount < rentExemptMin) {
          throw new Error(
            `Recipient SOL account does not exist yet; amount must be at least the rent-exempt minimum (${rentExemptMin} lamports).`,
          );
        }
      }
    }
